...
class _NoteDetailViewState extends State<NoteDetailView> {
  ...
  bool _hasLinks(Map<String, dynamic> l) =>
      _detailSocialValue(l, 'instagram').isNotEmpty ||
      _detailSocialValue(l, 'linkedin').isNotEmpty ||
      _detailSocialValue(l, 'x').isNotEmpty ||
      _detailSocialValue(l, 'tiktok').isNotEmpty ||
      _detailSocialValue(l, 'snapchat').isNotEmpty ||
      _detailSocialValue(l, 'facebook').isNotEmpty;

  String _detailSocialValue(Map<String, dynamic> links, String platform) {
    if (platform == 'x') {
      return (links['x'] ?? links['twitter'] ?? '').toString().trim();
    }
    return (links[platform] ?? '').toString().trim();
  }

  Map<String, dynamic> _detailSocialBrand(String platform) {
    switch (platform) {
      case 'instagram':
        return {
          'icon': FontAwesomeIcons.instagram,
      ...
      default:
        return {'icon': FontAwesomeIcons.link, 'color': Colors.grey};
    }
  }

  String _toUrl(String? h, String platform) {
    if (h == null || h.isEmpty) return '';
    if (h.startsWith('http')) return h;
    final handle = h.replaceAll('@', '');
    switch (platform) {
      case 'instagram':
  ...
  Widget _detailStatTile(String title, String value) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 12),
      decoration: BoxDecoration(
        color: AppColors.background,
  ...
  Widget _detailBioCard(String bio) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(horizontal: 2),
      child: Text(
  ...
  Widget _socialIcon(BuildContext ctx, String platform, String url) {
    final brand = _detailSocialBrand(platform);
    final Color color = brand['color'] as Color;
    final IconData icon = brand['icon'] as IconData;
    return Padding(
